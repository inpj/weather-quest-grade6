...
- 單選題、複選題、拖曳排序題
- 閱讀情境、資料判讀、實驗推理、防災決策
- 自製水循環、等壓線、鋒面、颱風、衛星資料示意圖
- 第 10 題進入 BOSS「氣象核心」戰
- 提示晶片：使用後答對得 80 XP；未使用答對得 100 XP
- 3 格能量、即時解析、本輪作答紀錄
- 五大區域學習表現分析
- 教師模式：查看 50 題題庫、題型與正確答案
- 學生作答結果可匯出 CSV / JSON
- localStorage 儲存本輪進度
- 響應式版面，可用電腦、平板與手機
...
## 拖曳閱讀素養題
V2 將部分題目轉換成拖曳排序任務，例如：
- 衛星雲圖資料產生流程
- 凱米颱風事件時序
- 提前預報到防災行動的決策流程

桌機可直接拖曳；觸控裝置也可依序點兩張卡片交換位置。
...
網站入口為根目錄 `index.html`。main 分支更新後，GitHub Pages 通常會自動重新部署。
...
## 教師模式注意
教師模式是方便課堂檢視與匯出資料的介面，不是安全的答案保護機制。因網站為公開的純前端靜態頁面，熟悉瀏覽器開發工具的使用者仍可能查看題庫原始碼。

## 素材策略
目前畫面使用 CSS、文字與 emoji 自製視覺，未直接內嵌課本掃描頁或原版插圖，以降低公開 repository 的教材著作權風險。